...
$count_query = "SELECT COUNT(*) as total FROM resep_kue";
...
$query = "SELECT r.*, k.nama_kue 
          FROM resep_kue r
          JOIN jenis_kue k ON r.id_jenis_kue = k.id_jenis_kue
          ORDER BY k.nama_kue, r.nama_resep, r.versi
...
<body data-pc-preset="preset-1" data-pc-direction="ltr" data-pc-theme="light">
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="d-flex justify-content-between align-items-center">
                            <h3 class="mb-0">Daftar Resep Kue</h3>
                            ...
                            <div class="col-md-4">
                                <form method="get" class="form-inline">
                                    <div class="input-group">
                                        ...
                                        <span class="input-group-text">data resep</span>
                                    </div>
                                </form>
                            </div>
                        </div>
                                ...
                                <tbody>
                                    ...
                                    <?php else: ?>
                                        <?php foreach ($resep as $i => $row): ?>
                                            <tr>
                                                <td><?= $offset + $i + 1 ?></td>
                                                <td><?= htmlspecialchars($row['nama_kue']) ?></td>
                                                <!-- <td><?= htmlspecialchars($row['nama_resep']) ?></td> -->
                                                <!-- <td><?= htmlspecialchars($row['versi']) ?></td> -->
                                                <!-- <td>± <?= $row['porsi'] ?></td> -->
                                                <td class="text-center">
                                                    <span class="badge bg-<?= $row['aktif'] ? 'success' : 'danger' ?>">
                                                        <?= $row['aktif'] ? 'Aktif' : 'Nonaktif' ?>
                                                    </span>
                                                </td>
                                                <td class="text-center">
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="edit.php?id=<?= $row['id_resep'] ?>" class="btn btn-warning" title="Edit">
                                                            <i class="fas fa-edit"></i>
                                                        </a>
                                                        <a href="delete.php?id=<?= $row['id_resep'] ?>" class="btn btn-danger btn-hapus-resep"
                                                            data-nama="Resep" title="Hapus">
                                                            <i class="fas fa-trash"></i>
                                                        </a>


                                                        <a href="detail.php?id=<?= $row['id_resep'] ?>" class="btn btn-info" title="Lihat Resep">
                                                            <i class="fas fa-file-alt"> | Lihat Resep</i>
                                                        </a>
                                                    </div>
                                                </td>
                                            </tr>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            document.querySelectorAll(".btn-hapus-resep").forEach(function(btn) {
