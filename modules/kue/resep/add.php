...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Cek apakah sudah ada resep dengan id_jenis_kue yang sama
    $stmt = $db->prepare("SELECT COUNT(*) FROM resep_kue WHERE id_jenis_kue = ?");
    $stmt->execute([$id_jenis_kue]);
    $count = $stmt->fetchColumn();

    if ($count > 0) {
        redirectWithMessage('add.php', 'danger', 'Resep untuk jenis kue ini sudah ada.');
        exit;
    }
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="id_jenis_kue" class="form-control" required>
                                            ...
                                            <?php foreach ($jenis_kue as $jk): ?>
                                                <?php
                                                // Cek apakah jenis kue ini sudah ada resepnya
                                                $stmt = $db->prepare("SELECT COUNT(*) FROM resep_kue WHERE id_jenis_kue = ?");
