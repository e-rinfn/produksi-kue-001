...
$stok_map = [];
foreach ($kue_tersedia as $k) {
    $stok_map[$k['id_jenis_kue']] = [
        'nama' => $k['nama_kue'],
        'stok' => $k['total_stok'],
        'harga' => $k['harga_jual']
    ];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $tanggal_penjualan = $_POST['tanggal_penjualan'] ?? date('Y-m-d H:i:s');
    ...
    $poin_dipakai = intval($_POST['poin_dipakai'] ?? 0);

    try {
        $db->beginTransaction();

        // 1. Validate stock availability first
        foreach ($kue as $i => $id_jenis_kue) {
            if (!isset($stok_map[$id_jenis_kue])) {
                throw new Exception("Jenis kue tidak valid: ID $id_jenis_kue");
            }

            if ($stok_map[$id_jenis_kue]['stok'] < $jumlah[$i]) {
                throw new Exception("Stok tidak mencukupi untuk " . $stok_map[$id_jenis_kue]['nama'] .
                    ". Stok tersedia: " . $stok_map[$id_jenis_kue]['stok'] .
                    ", dibutuhkan: " . $jumlah[$i]);
            }
        }
        ...
        // 3. Process each cake item
        foreach ($kue as $i => $id_jenis_kue) {
            $sisa_jumlah = $jumlah[$i];

            // Get available stock batches (FIFO)
            $stmt = $db->prepare("SELECT id_stok_kue, jumlah 
                                FROM stok_kue 
                                WHERE id_jenis_kue = ? AND jumlah > 0 
                                ORDER BY tanggal_kadaluarsa ASC");
            $stmt->execute([$id_jenis_kue]);

            while ($stok = $stmt->fetch() && $sisa_jumlah > 0) {
                $ambil = min($stok['jumlah'], $sisa_jumlah);

                $items[] = [
                    'id_jenis_kue' => $id_jenis_kue,
                    'id_stok_kue' => $stok['id_stok_kue'],
                    'jumlah' => $ambil,
                    'harga_satuan' => $harga[$i],
                    'diskon_satuan' => $diskon[$i] ?? 0,
                    'subtotal' => ($harga[$i] - ($diskon[$i] ?? 0)) * $ambil,
                    'poin_diberikan' => ($poin_input[$i] ?? 0) * $ambil
                ];

                $total_harga += ($harga[$i] - ($diskon[$i] ?? 0)) * $ambil;
                $total_poin += ($poin_input[$i] ?? 0) * $ambil;
                $sisa_jumlah -= $ambil;
            }

            if ($sisa_jumlah > 0) {
                // This shouldn't happen since we validated stock earlier
                throw new Exception("Terjadi ketidaksesuaian stok untuk " . $stok_map[$id_jenis_kue]['nama']);
            }
        }

        // 4. Handle customer points
        ...
        if (!$id_penjualan) {
            throw new Exception("Gagal membuat record penjualan");
        }

        // 6. Insert sale details and update stock
        ...
        // 7. Update customer points if applicable
        if ($id_pelanggan) {
            // Add earned points
            if ($total_poin > 0) {
    ...
    <script>
        $(document).ready(function() {
                       ...
                       <select name="kue[]" class="form-control select-kue" required>
                            <?php foreach ($kue_tersedia as $k): ?>
                                <option value="<?= $k['id_jenis_kue'] ?>"
                                    ...
                                    (Stok: <?= $k['total_stok'] ?>, Harga: <?= number_format($k['harga_jual']) ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
            ...
            function initializeItemRow(row) {
                ...
                // Dalam fungsi initializeItemRow
