...
// Ambil data kue: total stok per jenis + id_stok_kue tertua
$sql = "
SELECT 
  j.id_jenis_kue,
  j.nama_kue,
  ...
  -- stok tertua: id, jumlah, exp
  (SELECT sk1.id_stok_kue
   FROM stok_kue sk1
   WHERE sk1.id_jenis_kue = j.id_jenis_kue
     AND sk1.jumlah > 0
     AND sk1.tanggal_kadaluarsa >= CURDATE()
  ...
  (SELECT sk2.jumlah
   ...
   WHERE sk2.id_jenis_kue = j.id_jenis_kue
     AND sk2.jumlah > 0
     AND sk2.tanggal_kadaluarsa >= CURDATE()
  ...
  (SELECT sk3.tanggal_kadaluarsa
   ...
   WHERE sk3.id_jenis_kue = j.id_jenis_kue
     AND sk3.jumlah > 0
     AND sk3.tanggal_kadaluarsa >= CURDATE()
   ORDER BY sk3.tanggal_kadaluarsa ASC, sk3.id_stok_kue ASC
   LIMIT 1
  ) AS exp_tertua
FROM jenis_kue j

-- total semua stok per jenis
JOIN (
    SELECT id_jenis_kue, SUM(jumlah) AS total_stock
    FROM stok_kue
    WHERE jumlah > 0 AND tanggal_kadaluarsa >= CURDATE()
